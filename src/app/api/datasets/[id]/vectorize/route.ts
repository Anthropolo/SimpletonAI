...
export async function POST(
  ...
  try {
    ...
    const command = `node ${cliPath} vectorize ${datasetId}`;
    ...
    if (stderr && stderr.includes('Error:')) {
      console.error('Vectorization error:', stderr);
    ...
    let errorMessage = error instanceof Error ? error.message : 'Failed to process vectorization request';
